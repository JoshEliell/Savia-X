    ...
    if form.is_valid():
        ...
        for row in reader:
            if Product.objects.filter(codigo=row[0]):
                codigo = Product.objects.get(codigo= row[0])
                #if Marca.objects.filter(nombre = row[1]):
                marcas = Marca.objects.filter(nombre__in=row[1])
                inventario, _ = Inventario.objects.get_or_create(producto=codigo,cantidad=row[2], price=row[3], comentario=row[4],distrito = perfil.distrito,complete=False)
                marcas_str = row[1]
                ...
                for marcas_name in marcas_names:
                    marcas, _ = Marca.objects.get_or_create(nombre=marcas_name)
                    inventario.marca.add(marcas)
                inventario.complete=True
                inventario.save()
                #else:
                #    messages.error(request,f'La marca no existe dentro de la base de datos, producto:{row[0]}')
            else:
                messages.error(request,f'El producto código:{row[0]} no existe dentro de la base de datos')
